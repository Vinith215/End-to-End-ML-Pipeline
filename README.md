...
Ingestion Layer: Processes DICOM files using pydicom to extract image physics metrics (contrast, noise).

ETL Layer: Cleans and aggregates data using Polars (Rust-based DataFrame library) for high performance.

Model Layer: Trains a LightGBM classifier on hospital usage patterns.

Ops Layer: Tracks experiments and metrics using MLflow.

Serving Layer: Exposes a REST API via FastAPI & Docker, ready for AWS.
